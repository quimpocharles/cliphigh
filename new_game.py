...
def main():
    ...
    print()
    print("[ Quarter start timestamps (recommended) ]")
    print("  Pause the video at the first second of each quarter (clock = 10:00)")
    print("  and note the video timestamp.  These eliminate inter-quarter drift")
    print("  so the system can generate highlights without manual calibration.")
    print("  Press Enter to skip any quarter you don't have yet.")
    q2_secs = ask_time("  Q2 start video time")
    q3_secs = ask_time("  Q3 start video time")
    q4_secs = ask_time("  Q4 start video time")
    quarter_starts = {2: q2_secs, 3: q3_secs, 4: q4_secs}
    print()
    ...
    for q, secs in quarter_starts.items():
        if secs is not None:
            m, s = secs // 60, secs % 60
            print(f"  Q{q} start  : {m}m{s:02d}s into video ({secs}s)")
        else:
            print(f"  Q{q} start  : not provided")
    ...
    # Build quarter-start anchor lines for config
    quarter_clock = f"{period_length}:00"
    quarter_start_lines = []
    for q, secs in quarter_starts.items():
        if secs is not None:
            quarter_start_lines.append(
                f"    ({secs}, {q}, \"{quarter_clock}\"),  # Q{q} start"
            )
    quarter_start_block = ("\n" + "\n".join(quarter_start_lines)) if quarter_start_lines else ""
...
"""
    ...
    # ── Next steps ────────────────────────────────────────────────────────────
    provided_qs = [q for q, s in quarter_starts.items() if s is not None]
    all_qs_provided = tipoff_secs is not None and len(provided_qs) == 3
    ...
    if all_qs_provided:
        print("  All 4 quarter starts provided — you can generate directly:")
        print()
        print("  1. Download VOD and generate all quarters:")
        print("     python3 vod_replay.py --quarters 1 2 3 4")
        print()
        print("  2. If any clips look off, calibrate that quarter:")
        print("     python3 vod_replay.py --calibrate --quarters N")
        print("     python3 vod_replay.py --quarters N --skip-download")
    else:
        missing = [q for q in (2, 3, 4) if q not in provided_qs]
        if missing:
            print(f"  Quarter starts missing for Q{'/Q'.join(str(q) for q in missing)}.")
            print("  Add them to CALIBRATION_ANCHORS in config.py as:")
            for q in missing:
                print(f"    (video_seconds, {q}, \"{quarter_clock}\"),  # Q{q} start")
            print()
        print("  1. Dry-run to check estimated timestamps:")
        print("     python3 vod_replay.py --dry-run --quarters 1 2 3 4")
        print()
        print("  2. Calibrate any quarter that looks off:")
        print("     python3 vod_replay.py --calibrate --quarters N")
        print()
        print("  3. Generate highlights (downloads VOD on first run):")
        print("     python3 vod_replay.py --quarters 1 2 3 4")
